...
The p-values for each feature are obtained from the summary(model)$coefficients[, "Pr(>|t|)"] statement. The p-value represents the significance of each coefficient, and if the p-value is less than 0.05, we can reject the null hypothesis H_0: H_0: beta_j = 0, indicating a significant association between the feature and the response.
